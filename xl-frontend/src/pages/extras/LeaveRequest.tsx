    ...
    <div className="min-h-full bg-slate-900 flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 pt-12 pb-4 bg-slate-800 border-b border-slate-700/60 sticky top-0 z-10">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-400">Apply & track leaves</p>
        </div>
        {!showNew && (
      ...
      <div className="flex-1 p-4">
        {showNew && (
          <div className="bg-slate-800 rounded-2xl border border-slate-700 p-4 mb-6 shadow-xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" required value={startDate} onChange={e => setStartDate(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none" />
                </div>
                <div>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Leave Type</label>
                <select value={leaveType} onChange={e => setLeaveType(e.target.value)}
                  className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2.5 text-white text-sm focus:border-sky-500 focus:outline-none appearance-none">
                  {LEAVE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </div>

              <div>
                <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Reason</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowNew(false)} className="flex-1 py-2.5 rounded-xl border border-slate-700 text-slate-300 font-semibold text-sm active:bg-slate-700">Cancel</button>
        ...
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 px-1">Leave History</h3>
        ...
        {loading ? (
          ...
          <div className="space-y-3 pb-6">
            {leaves.map(leave => {
              ...
              return (
                <div key={leave._id} className="bg-slate-800 rounded-2xl p-4 border border-slate-700/50 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 bg-slate-900 px-2 py-0.5 rounded-full">{leave.leaveType}</span>
                    </div>
                  ...
                  {leave.adminRemarks && (
                    <div className="p-2.5 bg-slate-900 rounded-xl border border-slate-800 flex gap-2 items-start mt-1">
                      <span className="text-xs font-bold text-slate-500 uppercase">Note:</span>
                      <p className="text-xs text-slate-400 leading-tight flex-1">{leave.adminRemarks}</p>
                    </div>
                  )}
                </div>
              );
