...
export default function GeoFencing() {
  ...
  const fetchDoctors = async () => {
    try {
      const res = await axios.get('/api/xl/entities?type=Doctor');
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        try {
          ...
          await axios.put(`/api/xl/doctor/${drId}/geo`, payload);
          await fetchDoctors();
  ...
  const pendingCount = doctors.filter(d => !d.lat1).length;
    ...
    <div className="min-h-full bg-slate-900 flex flex-col">
      ...
      <div className="flex items-center gap-3 px-4 pt-12 pb-4 bg-slate-800 border-b border-slate-700/60 sticky top-0 z-10">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-400">Manage doctor locations</p>
        </div>
      </div>
        ...
        {pendingCount > 0 && (
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 mb-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-400/80 mt-0.5">You have {pendingCount} doctors without a primary location. DCR submission is blocked for them.</p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-3 flex-1 overflow-y-auto pb-6">
          {loading ? (
            ...
            filteredDoctors.map(dr => (
              <div key={dr._id} className="bg-slate-800 rounded-2xl p-4 border border-slate-700/50">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-base font-bold text-white">{dr.name}</h3>
                    <p className="text-xs text-slate-400">{dr.specialization || 'General'} • {dr.workingArea || dr.hq}</p>
                  </div>
                  {dr.lat1 && dr.lat2 ? (
                    <span className="text-[10px] font-bold text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-full uppercase tracking-wider">Fully Tagged</span>
                  ) : dr.lat1 ? (
                    <span className="text-[10px] font-bold text-sky-400 bg-sky-400/10 px-2 py-1 rounded-full uppercase tracking-wider">Primary Tagged</span>
                  ) : (
                    <span className="text-[10px] font-bold text-rose-400 bg-rose-400/10 px-2 py-1 rounded-full uppercase tracking-wider">Not Tagged</span>
                  )}
                </div>
                  ...
                  {dr.lat1 ? (
                    <div className="bg-slate-900 border border-emerald-500/30 rounded-xl p-2.5 flex items-center gap-2">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-[10px] text-slate-400 truncate">{dr.geoAddress1}</p>
                      </div>
                    </div>
                  ) : (
                    <button onClick={() => handleTag(dr._id, 1)} disabled={tagLoading !== null} className="h-[46px] rounded-xl border border-dashed border-slate-600 flex flex-col items-center justify-center gap-0.5 active:bg-slate-700 disabled:opacity-50">
                      {tagLoading === `${dr._id}-1` ? <Navigation size={12} className="animate-spin text-emerald-400" /> : <MapPin size={12} className="text-emerald-400" />}
                  ...
                  {dr.lat2 ? (
                    <div className="bg-slate-900 border border-sky-500/30 rounded-xl p-2.5 flex items-center gap-2">
                      ...
                      <div className="overflow-hidden">
                        ...
                        <p className="text-[10px] text-slate-400 truncate">{dr.geoAddress2}</p>
                      </div>
                    </div>
                  ) : (
                    <button onClick={() => handleTag(dr._id, 2)} disabled={tagLoading !== null} className="h-[46px] rounded-xl border border-dashed border-slate-600 flex flex-col items-center justify-center gap-0.5 active:bg-slate-700 disabled:opacity-50">
                      {tagLoading === `${dr._id}-2` ? <Navigation size={12} className="animate-spin text-sky-400" /> : <MapPin size={12} className="text-sky-400" />}
