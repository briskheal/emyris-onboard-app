...
const ReportsTab: React.FC<ReportsTabProps> = ({ initialTab = 'details', isStandalone = false }) => {
  ...
  const getApplicantRow = (app: any) => {
    ...
    const fullAddress = [fd.address, fd.city, fd.state, fd.pin].filter(Boolean).join(", ");
    ...
    return [
      ...
      fd.dob || "",
      fd.gender || "",
      app.maritalStatus || fd.maritalStatus || "",
      app.anniversaryDate || fd.anniversaryDate || "",
      app.bloodGroup || fd.bloodGroup || "",
      fullAddress,
      fd.highestQualification || fd.qualification || "",
      fd.passingYear || fd.yearOfPassing || "",
      fd.previousCompany || fd.lastCompany || "",
      fd.totalExperience || "",
      fd.salary ? `INR ${fd.salary}` : "",
      fd.bankName || "",
      fd.accNo || fd.accountNumber || "",
      fd.ifsc || fd.ifscCode || "",
      fd.panNumber || "",
      fd.aadharNumber || "",
      app.epfNumber || fd.epfNumber || "",
      app.uanNumber || fd.uanNumber || "",
      app.esiNumber || fd.esiNumber || "",
      ...
      app.actualJoiningDate ? new Date(app.actualJoiningDate).toLocaleString() : ""
    ];
  };
                  ...
                  <tr key={idx} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    ...
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                      <span className="badge" style={{ background: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa' }}>
                        {exam.autoScore || 0} / {exam.mcqTotal || exam.totalQuestions || 0}
                      </span>
                    </td>
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                      <span className="badge" style={{ background: 'rgba(245, 158, 11, 0.15)', color: '#fbbf24' }}>
                        {exam.manualScore || 0} / {exam.descTotal || 0}
                      </span>
                    </td>
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                ...
                <div style={{ width: '100%', maxWidth: '780px', maxHeight: '88vh', overflowY: 'auto', padding: '2.25rem', background: '#0f172a', border: '1px solid #334155', borderRadius: '20px', boxShadow: '0 25px 60px -15px rgba(0, 0, 0, 0.95)', borderTop: '6px solid #3b82f6' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid #334155', paddingBottom: '1.25rem' }}>
                    <div>
                      ...
                      <p style={{ color: '#94a3b8', fontSize: '0.9rem', margin: '6px 0 0 0', fontWeight: 500 }}>
                        Email: {selectedExamDetail.email} | Total Score: <span style={{ color: '#60a5fa', fontWeight: 800 }}>{selectedExamDetail.totalScore || 0} / {(selectedExamDetail.mcqTotal || 0) + (selectedExamDetail.descTotal || 0) || selectedExamDetail.totalQuestions || 20} Points</span>
                      </p>
                    </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                    <div style={{ background: '#1e293b', border: '1px solid #3b82f6', padding: '1.25rem', borderRadius: '12px', textAlign: 'center', boxShadow: '0 4px 15px rgba(59,130,246,0.15)' }}>
                      ...
                      <div style={{ fontSize: '1.6rem', fontWeight: 900, color: '#60a5fa', marginTop: '6px' }}>{selectedExamDetail.autoScore || 0} / {selectedExamDetail.mcqTotal || selectedExamDetail.totalQuestions || 20} Points</div>
                    </div>
                    <div style={{ background: '#1e293b', border: '1px solid #f59e0b', padding: '1.25rem', borderRadius: '12px', textAlign: 'center', boxShadow: '0 4px 15px rgba(245,158,11,0.15)' }}>
                      ...
                      <div style={{ fontSize: '1.6rem', fontWeight: 900, color: '#fbbf24', marginTop: '6px' }}>{selectedExamDetail.manualScore || 0} / {selectedExamDetail.descTotal || 0} Points</div>
                    </div>
                  </div>
                      ...
                      return (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                          {Object.entries(descAns).map(([qId, ans]: any, i: number) => {
                            ...
                            const isMCQ = qInfo ? qInfo.questionType === 'mcq' : (typeof ans === 'number' || (!isNaN(parseInt(ans)) && ans.length < 3));
