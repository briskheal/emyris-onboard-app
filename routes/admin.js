...
router.put('/dcs/doctors/:id', async (req, res) => { try { await XlDoctor.update(req.body, { where: { _id: req.params.id } }); if (req.body.lat1 === null) { const { XlGeoFencing } = require('../db'); await XlGeoFencing.destroy({ where: { entityId: req.params.id, entityType: 'Doctor' } }); } res.json({ success: true }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } });
...
router.put('/dcs/chemists/:id', async (req, res) => { try { await XlChemist.update(req.body, { where: { _id: req.params.id } }); if (req.body.lat1 === null) { const { XlGeoFencing } = require('../db'); await XlGeoFencing.destroy({ where: { entityId: req.params.id, entityType: 'Chemist' } }); } res.json({ success: true }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } });
...
router.put('/dcs/stockists/:id', async (req, res) => { try { await XlStockist.update(req.body, { where: { _id: req.params.id } }); if (req.body.lat1 === null) { const { XlGeoFencing } = require('../db'); await XlGeoFencing.destroy({ where: { entityId: req.params.id, entityType: 'Stockist' } }); } res.json({ success: true }); } catch (e) { res.status(500).json({ success: false, message: e.message }); } });
...
router.get('/xl-backlog', async (req, res) => {
    try {
        const { XlBacklogRequest, XlUser } = require('../db');
        const requests = await XlBacklogRequest.findAll({ order: [['createdAt', 'DESC']] });
        
        // Join with employee names
        const employees = await XlUser.findAll();
        const empMap = {};
        employees.forEach(e => {
            empMap[e.uid] = e.firstName + ' ' + (e.lastName || ''); empMap[e.email] = e.firstName + ' ' + (e.lastName || ''); empMap[e.employeeId] = e.firstName + ' ' + (e.lastName || '');
        });
