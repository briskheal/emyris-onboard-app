console.log('≡ƒôü script.js: Primary Execution Trap Active');
let currentStep = 1;
let isSaving = false;
...
async function downloadDatabase() {
    try {
        ...
        showToast("Γ£à Database export successful", "success");
    } catch (e) { showToast("Γ¥î Export failed", "error"); }
    finally { unlockUI(); }
}
...
async function nukeDatabase() {
    const confirm1 = confirm("Γÿó∩╕Å WARNING: This will PERMANENTLY delete all Applicant data and reset letter counters. Proceed?");
    if (!confirm1) return;
    const confirm2 = prompt("ΓÜá∩╕Å NUCLEAR SAFETY CHECK: Type 'DELETE ALL' to confirm the total wipe.");
    if (confirm2 !== "DELETE ALL") {
        showToast("Γ¥î Clear cancelled. Safety check failed.", "error");
        return;
    }
    const confirm3 = confirm("Γ¥ô Also clear all Divisions and HQs? (Click 'OK' for a total fresh start)");
    
    try {
        lockUI("Γÿó∩╕Å Nuking Database... Please Wait");
        ...
        if ((await res.json()).success) {
            showToast("Γ£à Database cleared successfully", "success");
            await fetchCompanyData();
            await fetchApplicants(); 
            switchAdminTab('profile'); 
        }
    } catch (e) { showToast("Γ¥î System wipe failed", "error"); }
    finally { unlockUI(); }
}
...
async function submitExistingStaff(event) {
    ...
    lockUI("ΓÜí Fast-Tracking Existing Staff...");
    ...
    try {
        ...
        if (result.success) {
            showToast("≡ƒÄë Existing staff member fast-tracked successfully!", "success");
...
// --- UI HELPERS ---
function lockUI(msg = "≡ƒÅï∩╕Å Processing... Please Wait") {
    isSaving = true;
...
function attachStatusListener(inputId, config) {
    ...
    if (el) el.addEventListener('change', () => {
        ...
        if (label && files.length > 0) {
            label.innerText = files.length === 1 ? `Γ£à ${files[0].name.substring(0, 15)}` : `Γ£à ${files.length} Files Selected`;
    ...
    if (list) {
        list.innerHTML = allHQs.map(h => `
            <div class="division-chip">
                <span>${h.name}</span>
                <button onclick="deleteHQ('${h._id}')">Γ£ò</button>
            </div>
        `).join('');
    }
...
async function addHQ() {
    ...
    try {
        ...
        if ((await res.json()).success) {
            input.value = "";
            fetchHQs();
            showToast("Γ£à HQ added successfully", "success");
        }
...
function applyCompanyData() {
    ...
    if (logoImg) {
        if (companyData.logo && companyData.logo.length > 0) {
            ...
            console.log('Γ£à Updated Hero Logo');
        } else {
            console.log('ΓÜá∩╕Å No Logo in companyData');
        }
    }
    ...
    const contactHTML = `
        ${companyData.phone ? `<div>≡ƒô₧ <a href="tel:${companyData.phone}" class="contact-link">${companyData.phone}</a></div>` : ''}
        ${companyData.tollFree ? `<div>ΓÿÄ∩╕Å Toll Free: <a href="tel:${companyData.tollFree}" class="contact-link">${companyData.tollFree}</a></div>` : ''}
        ${companyData.website ? `<div>≡ƒîÉ <a href="${companyData.website}" target="_blank" class="contact-link">${companyData.website.replace('https://', '')}</a></div>` : ''}
    `;
    ...
    if (headerTitle) {
        ...
        console.log('Γ£à Updated Header Name:', headerTitle.innerText);
    }
    ...
    if (companyData.logo && companyData.logo.length > 0 && headerImg) {
        ...
        console.log('Γ£à Updated Header Logo');
    } else {
    ...
    } catch (e) { console.error('Γ¥î Error in child layout functions:', e); }
    ...
    console.log('Γ£à Marquees Synchronized with Profile Settings');
}
...
function renderRequiredDocsSuggestions() {
    ...
    STANDARD_DOCS.forEach(doc => {
        ...
        chip.innerHTML = `<span>${isSelected ? 'Γ£ô ' : '+ '}${doc}</span>`;
...
function renderApplicantDocuments() {
        ...
        docs.forEach(docName => {
                    ...
                    <div class="uploaded-files-list">
                        ${categoryDocs.map((d, idx) => `
                            <div class="file-item-pill">
                                <div class="status-icon-pending">≡ƒÅï∩╕Å</div>
                                <span class="file-name">≡ƒôä ${d.name}</span>
            ...
            box.innerHTML = `
                ...
                <div class="drop-zone ${hasUploads ? 'has-files' : ''}" onclick="document.getElementById('file_${safeId}').click()">
                    ...
                    <span class="drop-icon">${hasUploads ? '≡ƒÅï∩╕Å' : '≡ƒôÄ'}</span>
    ...
    sigBox.innerHTML = `
        ...
        <div class="drop-zone" onclick="document.getElementById('file_Signature').click()">
            ...
            <span class="drop-icon">${hasSig ? 'Γ£ì∩╕Å Γ£à' : 'Γ£ì∩╕Å'}</span>
        ...
        ${hasSig ? `
            <div class="uploaded-files-list">
                <div class="file-item-pill">
                    <span class="file-name">≡ƒûï∩╕Å ${sigDocs[0].name}</span>
...
async function deleteApplicantDoc(assetId, category) {
    ...
    try {
        lockUI("≡ƒÅï∩╕Å Removing Document...");
        ...
        if (result.success) {
            ...
            showToast("≡ƒùæ∩╕Å Document removed", "success");
        }
    } catch (e) { showToast("Γ¥î Deletion failed", "error"); }
    finally { unlockUI(); }
}
...
function attachApplicantFileListener(inputId, category) {
    ...
    input.addEventListener('change', async (e) => {
        ...
        if (fileSizeMB > maxSizeMB) {
            showToast(`Γ¥î Too large. Max ${maxSizeMB}MB.`, "error");
            return;
        }

        if (label) label.innerText = `≡ƒÅï∩╕Å Uploading...`;
        if (ribbon) { 
        ...
        try {
            ...
            if (result.success) {
                if (label) { 
                    label.innerText = `Γ£à ${category} Uploaded`; 
                    label.style.color = "var(--success)"; 
                }
                if (ribbon) { 
                ...
                showToast(`Γ£à ${category} saved!`, "success");
            } else { 
            ...
            if (label) { 
                label.innerText = `Γ¥î ${errMsg.substring(0, 30)}...`; 
            ...
            showToast(`Γ¥î Upload failed: ${errMsg}`, "error");
            input.value = '';
        } finally {
...
async function handleApplicantRegister(e) {
    ...
    if (missing.length > 0) {
        showToast("ΓÜá∩╕Å Please fill all required fields", "warning");
    ...
    try {
        lockUI("≡ƒ¢í∩╕Å Creating Application...");
        ...
        if (result.success || result.needsRecovery) {
            ...
            alert(`Γ£à Registration Successful!\n\nYOUR LOGIN PIN: ${pin}\n\nPlease note this down now. We have also sent it to: ${data.email}`);
            document.getElementById('loginEmail').value = data.email;
            updateView('applicantLogin');
        } else {
            showToast(`Γ¥î Error: ${result.message}`, 'error');
        }
    } catch (err) { 
        showToast("Γ¥î Server error during registration.", 'error'); 
    }
    finally { unlockUI(); }
}
...
async function handleApplicantLogin(e) {
    ...
    try {
        ...
        if (result.success) {
            ...
            showToast(`Γ¥î ${result.message}`, 'error');
        }
    } catch (err) { 
        showToast("Γ¥î Login failed. Check connection.", 'error'); 
    }
}
...
async function handleForgotPin() {
    const email = document.getElementById('loginEmail').value;
    if (!email) {
        showToast('ΓÜá∩╕Å Please enter your registered email address first.', 'warning');
    ...
    try {
        ...
        showToast('Γ¥î Recovery failed. Check connection.', 'error');
    } finally {
        unlockUI();
    }
}
...
function renderApplicantDashboard() {
    ...
    document.getElementById('dash_email').innerText = `≡ƒôº ${app.email}`;
    ...
    timeline.innerHTML = steps.map((s, i) => `
        <div class="timeline-item-premium ${s.done ? 'done' : ''}">
            <div class="timeline-dot-premium">${s.done ? 'Γ£ô' : i + 1}</div>
                ...
                <div class="doc-status-row ${isRejected ? 'rejected-mode' : ''}" style="flex-direction: column; align-items: stretch; gap: 0.75rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div class="doc-actions">
                            ${!isApproved ? `<button class="btn btn-sm btn-outline" onclick="triggerDocResubmit('${dName}')" style="border-color: var(--primary); color: var(--primary);">≡ƒÅï∩╕Å ${categoryFiles.length > 0 ? 'Add More' : 'Upload Now'}</button>` : ''}
                            ${isApproved ? '<span style="color:var(--success)">Γ£à</span>' : ''}
                        </div>
                    </div>
...
async function triggerDocResubmit(category) {
    ...
    input.onchange = async (e) => {
        ...
        try {
            lockUI(`≡ƒÅï∩╕Å Resubmitting ${category}...`);
            const reader = new FileReader();
            reader.onload = async () => {
                ...
                const result = await res.json();
                if (result.success) {
                    showToast("Γ£à Document resubmitted successfully!", "success");
...
async function acceptOfferLetter() {
    ...
    if (!adoj) {
        showToast('ΓÜá∩╕Å Please select your Actual Date of Joining first.', 'warning');
    ...
    try {
        ...
        if ((await res.json()).success) {
            showToast('≡ƒÄë Congratulations! Welcome to the family.', 'success');
            currentApplicant.offerAccepted = true;
            currentApplicant.actualJoiningDate = adoj;
            renderApplicantDashboard();
        } else {
            showToast('Γ¥î Acceptance failed. Please try again.', 'error');
        }
    } catch (e) { 
        showToast('Γ¥î Network error. Please try again.', 'error');
    }
    finally { unlockUI(); }
}
...
function nextStep(step) {
    ...
    if (!isValid) return showToast("ΓÜá∩╕Å Please fill all required fields", "warning");
    ...
    if (currentStep === 5) {
        ...
        if (missing.length > 0) {
            showToast(`ΓÜá∩╕Å Missing: ${missing.slice(0, 2).join(', ')}${missing.length > 2 ? ` +${missing.length-2} more` : ''}`, 'warning');
            return;
        }
    }
...
function showReview() {
    ...
    const groups = {
        "≡ƒæÑ Personal": ["firstName", "lastName", "dob", "gender", "bloodGroup", "fatherName"],
        "≡ƒÆ╝ Employment": ["designation", "joiningDate", "salary", "hq"],
        "≡ƒô₧ Contact": ["phone", "address", "city", "state", "pin"],
        "≡ƒÅª Bank": ["bankName", "accNo", "ifsc"]
    };
        ...
        return `
            <div class="review-item">
                ...
                <span class="review-value" style="color: ${up ? 'var(--success)' : '#ef4444'}">
                    ${up ? 'Γ£à Already Uploaded' : 'Γ¥î NOT UPLOADED'}
                </span>
            </div>
        `;
    }).join('');
    ...
    docItems += `
        <div class="review-item">
            ...
            <span class="review-value" style="color: ${sig ? 'var(--success)' : '#ef4444'}">
                ${sig ? 'Γ£à Already Uploaded' : 'Γ¥î NOT UPLOADED'}
            </span>
        </div>
    `;
    ...
    reviewContent.innerHTML += `
        <div class="review-section-group">
            <h4>≡ƒôü Uploaded Testimonials</h4>
...
async function handleAdminLogin() {
    ...
    if ((await res.json()).success) {
        ...
        showToast('Γ¥î Invalid Admin Credentials', 'error');
...
async function initializeApp() {
    console.log('≡ƒÜÇ Emyris App initialized v1.3');
    await fetchCompanyData();
    ...
    setTimeout(() => {
        if (wasAdmin) {
            updateView('adminDashboard');
            switchAdminTab('profile');
            console.log('≡ƒöä Session Recovered: Admin Dashboard');
        } else {
            updateView('landingPage');
            console.log('Γ£à View state synchronized: landingPage');
        }
    }, 150);
...
async function submitProfileUpdate(data, silent = false) {
    if (!silent) lockUI("≡ƒÅï∩╕Å Saving Changes...");
    try {
        ...
        const result = await res.json();
        if (result.success) {
            showToast('Γ£à Configuration Updated Successfully!', 'success');
            // Reset ribbons
            ...
            showToast('Γ¥î Save failed. Please try again.', 'error');
        }
    } catch (err) { 
        console.error("Save error:", err);
        showToast('Γ¥î Connection error. Save failed.', 'error'); 
    } finally { 
...
function attachAssetUploadListener(id, info) {
    ...
    el.onchange = async (e) => {
        ...
        try {
            ...
            showToast(`Γ£à ${files.length} file(s) uploaded!`);
            await fetchCompanyData();
            renderAssetLists();
        } catch (err) {
            showToast("Γ¥î Upload failed", "error");
        } finally {
            if (ribbon) {
                setTimeout(() => {
...
function switchAdminTab(tab) {
    if (typeof isSaving !== 'undefined' && isSaving) {
        showToast("ΓÜá∩╕Å Please wait until the current save is completed.", "error");
        return;
    }
...
async function fetchApplicants() {
    try {
        ...
        if (Array.isArray(data)) {
            ...
            showToast("ΓÜá∩╕Å Could not load applicant data", "error");
        }
    } catch (err) {
...
async function renderAssetLists() {
    try {
                ...
                companyData.customAssetCategories.forEach(cat => {
                    ...
                    box.innerHTML = `
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.2rem;">
                            ...
                            <button type="button" onclick="deleteAssetCategory('${cat}')" style="background: none; border: none; color: #ef4444; cursor: pointer; font-size: 0.75rem; opacity: 0.6;" title="Remove Category">≡ƒùæ∩╕Å Delete</button>
                        </div>
                        <div class="drop-zone-sm" onclick="document.getElementById('input_${safeKey}').click()">
                            <span class="drop-icon-sm">≡ƒôÄ</span>
            ...
            item.innerHTML = `
                <div class="asset-mini-preview">
                    ${asset.data.startsWith('data:image') 
                        ...
                        : `<span style="font-size:12px">≡ƒôä</span>`}
                </div>
                ...
                <div class="asset-item-actions">
                    ...
                    <button class="btn-asset-mini delete-btn" onclick="deleteAssetRecord('${asset._id}')">≡ƒùæ∩╕Å</button>
                </div>
            `;
        ...
        const updateStatus = (id, count, label) => {
            const el = document.getElementById(id);
            if (!el) return;
            el.innerText = count ? `Γ£à ${count} ${label}(s) Uploaded` : `Upload ${label}`;
        ...
        if (companyData.customAssetCategories) {
            companyData.customAssetCategories.forEach(cat => {
                ...
                if (el) {
                    ...
                    el.innerText = count ? `Γ£à ${count} File(s) Uploaded` : `Upload Files`;
                    el.style.color = count ? 'var(--success)' : 'var(--text-muted)';
                }
            });
        }
...
async function addAssetCategory() {
    ...
    try {
        lockUI("≡ƒÅù∩╕Å Creating Category...");
        ...
        if ((await res.json()).success) {
            showToast(`Γ£à Category ${categoryName} created!`);
            input.value = '';
            await fetchCompanyData();
            renderAssetLists();
        }
    } catch (e) { showToast("Γ¥î Creation failed", "error"); }
    finally { unlockUI(); }
}
...
async function deleteAssetCategory(categoryName) {
    ...
    try {
        lockUI("≡ƒùæ∩╕Å Removing Category...");
        ...
        const result = await res.json();
        if (result.success) {
            showToast(`Γ£à Category ${categoryName} removed!`);
            await fetchCompanyData();
            renderAssetLists();
        } else {
            showToast(result.message || "Γ¥î Deletion failed", "error");
        }
    } catch (e) { showToast("Γ¥î Delete failed", "error"); }
    finally { unlockUI(); }
}

async function setActiveAsset(assetId, category) {
    try {
        lockUI("ΓÜÖ∩╕Å Updating Active Asset...");
        ...
        if ((await res.json()).success) {
            showToast("Γ£à Active asset updated!", "success");
            await fetchCompanyData(); // Refresh global pointers
            renderAssetLists(); // Refresh lists
        }
    } catch (e) { showToast("Γ¥î Update failed", "error"); }
    finally { unlockUI(); }
}
...
async function deleteAssetRecord(assetId) {
    ...
    try {
        ...
        if ((await res.json()).success) {
            showToast("Γ£à Asset deleted!", "success");
            await fetchCompanyData();
            renderAssetLists();
        }
    } catch (e) { showToast("Γ¥î Delete failed", "error"); }
                ...
                <td style="text-align: right; white-space: nowrap;">
                    ${(() => {
                        if (app.status === 'approved') {
                            return `<button class="btn btn-sm" onclick="openVerificationView('${app.email}')" style="background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.3); color: #10b981; padding: 6px 12px; font-weight: 700; border-radius: 8px; font-size: 0.75rem;">≡ƒùé∩╕Å DATABASE</button>`;
                        } else if (app.status === 'rejected') {
                            return `<button class="btn btn-sm" onclick="openVerificationView('${app.email}')" style="background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.2); color: #ef4444; padding: 6px 12px; font-weight: 700; border-radius: 8px; font-size: 0.75rem;">≡ƒò╡∩╕Å AUDIT</button>`;
                        } else {
                            return `<button class="btn btn-sm btn-primary" onclick="openVerificationView('${app.email}')" style="background: var(--accent); border-color: var(--accent); padding: 6px 12px; font-weight: 700; border-radius: 8px; font-size: 0.75rem;">≡ƒöÄ VERIFY</button>`;
                        }
                    })()}
                </td> (Checked)
            </tr>
...
async function deleteApplicant(email) {
    if (!confirm(`≡ƒÜ¿ CRITICAL: Are you sure you want to permanently delete applicant ${email} and all their uploaded testimonials? This cannot be undone.`)) return;
    
    try {
        lockUI("≡ƒùæ∩╕Å Deleting Record...");
        ...
        const result = await res.json();
        if (result.success) {
            showToast("Γ£à Applicant and assets deleted successfully.", "success");
            await fetchApplicants(); // Refresh table
        } else {
            showToast("Γ¥î Delete failed: " + result.error, "error");
        }
    } catch (e) { showToast("Γ¥î Server communication error", "error"); }
    finally { unlockUI(); }
}
...
async function openVerificationView(email) {
    ...
    if (!sal.basic && app.formData?.salary && parseFloat(app.formData.salary) > 0) {
        console.log("ΓÜí Auto-calculating initial salary breakup for admin...");
        ...
        if (app.offerAccepted) {
            accNote.innerHTML = `
                <div style="background: rgba(16, 185, 129, 0.1); border: 1px solid var(--success); border-radius: 12px; padding: 1rem; margin-bottom: 1rem;">
                    <h5 style="color: var(--success); margin: 0 0 5px 0; font-size: 0.9rem;">≡ƒÄë OFFER ACCEPTED</h5>
        ...
        if (app.status === 'rejected') {
            rejNote.innerHTML = `
                <div style="background: rgba(239, 68, 68, 0.1); border: 1px solid #ef4444; border-radius: 12px; padding: 1rem; margin-bottom: 1rem;">
                    <h5 style="color: #ef4444; margin: 0 0 5px 0; font-size: 0.9rem;">≡ƒÜ½ APPLICATION REJECTED</h5>
                    <p style="font-size: 0.8rem; color: var(--text-main); margin: 0;">
...
function resetVerificationUI() {
    ...
    if (totalEl) totalEl.innerText = 'Γé╣0';
    if (annualEl) annualEl.innerText = 'Γé╣0';
...
function renderVerificationProfile(app) {
    ...
    const rows = [
        ...
        { label: 'Offer Status', val: app.offerAccepted ? '<span style="color:var(--success); font-weight:bold;">Γ£à ACCEPTED</span>' : (app.status === 'approved' ? 'Issued (Pending)' : 'Not Issued') },
        ...
        { 
            ...
            val: `
                <div style="display:flex; gap:0.5rem; margin-top:5px; flex-wrap:wrap;">
                    ${app.offerLetterData ? `<button class="btn btn-sm btn-outline" onclick="previewIssuedLetter('${app.email}', 'offer')" style="padding:4px 8px; font-size:0.7rem; color:var(--accent); border-color:var(--accent);">≡ƒôä OFFER</button>` : ''}
                    ${app.apptLetterData ? `<button class="btn btn-sm btn-outline" onclick="previewIssuedLetter('${app.email}', 'appt')" style="padding:4px 8px; font-size:0.7rem; color:var(--primary-light); border-color:var(--primary-light);">≡ƒô£ APPT</button>` : ''}
            ...
            <div class="v-check-item ${isVerified ? 'verified' : (hasFiles ? 'waiting' : 'missing')}">
                <div class="v-check-info">
                    ...
                    <label style="font-size:0.7rem; color:${hasFiles ? 'var(--success)' : '#ef4444'}">
                        ${hasFiles ? `Γ£à ${categoryFiles.length} File(s)` : 'Γ¥î Missing File'}
                    </label>
                    <div class="v-check-file-list" style="margin-top: 5px;">
                        ${categoryFiles.map(f => `
                            <div style="font-size: 0.7rem; color: var(--text-soft); display: flex; align-items: center; gap: 8px; margin-bottom: 2px;">
                                <span>≡ƒôä ${f.name}</span>
                                <div style="display:flex; gap: 4px;">
                                    <button class="btn btn-tool" onclick="viewDocument('${f.assetId || ''}')" style="padding: 2px 5px; font-size: 0.65rem;">≡ƒæü∩╕Å</button>
                                    <button class="btn btn-tool" onclick="downloadAsset('${f.assetId || ''}', '${dName}')" style="padding: 2px 5px; font-size: 0.65rem;">≡ƒôÑ</button>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                ...
                <div class="v-check-actions" style="display:flex; align-items:center; gap:0.5rem;">
                    ${hasFiles ? `<button class="btn btn-tool btn-tool-danger" onclick="rejectDocument('${dName}')" title="Reject Category">≡ƒÜ⌐</button>` : ''}
...
async function rejectDocument(docCategory) {
    ...
    try {
        lockUI(`≡ƒÜ⌐ Rejecting ${docCategory}...`);
        ...
        const result = await res.json();
        if (result.success) {
            showToast(`Γ£à ${docCategory} rejected. Applicant notified.`, "success");
        ...
        if (categoryFiles.length === 0) {
            galleryItems.push(`
                <div class="doc-preview-card missing">
                    <div class="doc-icon">Γ¥î</div>
                    <div class="doc-name">${dName}</div>
                    <div class="doc-status-tag" style="background:rgba(239,68,68,0.12); color:#ef4444">Γ¥î Missing</div>
            ...
            categoryFiles.forEach(f => {
                ...
                galleryItems.push(`
                    <div class="doc-preview-card uploaded">
                        <div class="doc-icon">${isPdf ? '≡ƒôä' : '≡ƒû╝∩╕Å'}</div>
                        <div class="doc-name">${dName}</div>
                        <div class="doc-status-tag" style="background:rgba(16,185,129,0.15); color:#10b981">
                            Γ£à ${f.name}
                        </div>
                        <div class="doc-actions-row">
                            <button class="btn-tool" onclick="viewDocument('${f.assetId || ''}')" title="View">≡ƒæü∩╕Å</button> 
                            <button class="btn-tool" onclick="downloadAsset('${f.assetId || ''}', '${f.name}')" title="Download">≡ƒôÑ</button>
                        </div>
                    </div>
                `);
            });
...
async function updatePipelineTask(taskName, isChecked) {
    ...
    try {
        ...
        showToast('Γ£à Pipeline updated', 'success');
...
async function saveInternalAssignment(silent = false) {
    ...
    try {
        ...
        const result = await res.json();
        if (result.success) {
            if (!silent) showToast("Γ£à Core Assignment & Salary Updated!", "success");
    ...
    if (activeV_Applicant && activeV_Applicant.formData && activeV_Applicant.formData.salary) {
        ...
        if (feedback) {
            ...
            if (diff < 0.1) { // Rounding margin for decimals
                feedback.innerHTML = "Γ£à Matches target annual salary perfectly.";
                feedback.style.background = "rgba(16,185,129,0.15)";
                feedback.style.color = "#10b981";
                feedback.classList.remove('error-feedback');
            } else {
                feedback.innerHTML = `ΓÜá∩╕Å Mismatch: Γé╣${annual.toLocaleString('en-IN')} vs Target Γé╣${targetAnnual.toLocaleString('en-IN')}`;
...
async function rejectApplicantFlow() {
    ...
    if (!reason.trim()) {
        return showToast("ΓÜá∩╕Å A reason is required for rejection.", "warning");
    }
...
async function togglePipelineStep(step, isChecked) {
    ...
    try {
        ...
        if ((await res.json()).success) {
            ...
            syncPipelineSwitches(activeV_Applicant.tasks);
            showToast(`≡ƒÜÇ Onboarding Step: ${step} updated`, "success");
        }
            ...
            html += `
                <div class="asset-card">
                    ...
                    <div class="asset-card-preview">
                        ${isImg ? `<img src="${val}">` : (isPdf ? `<span class="pdf-icon">≡ƒôä</span>` : '?')}
                    </div>
                    <div class="asset-card-body">
                        <span class="asset-card-name">${obj.name || cat.name + ' #' + (idx + 1)}</span>
                        <div class="asset-card-action">
                            <button class="btn-asset-view" onclick="viewAssetRaw('${val}')">${isImg ? '≡ƒæü∩╕Å View' : 'Γ¼ç∩╕Å Download'}</button>
                        </div>
                    </div>
                </div>
            `;
...
async function deleteAsset(category, assetId) {
    ...
    try {
        ...
        if ((await res.json()).success) {
            showToast("≡ƒùæ∩╕Å Asset deleted successfully", "success");
...
async function loadSetupData() {
    ...
    if (companyData.letterheadImage && companyData.letterheadImage.length > 0 && lhStatus) {
        lhStatus.innerText = "Letterhead Uploaded Γ£à";
    ...
    if (companyData.stamp && companyData.stamp.length > 0 && stStatus) {
        stStatus.innerText = "Stamp Uploaded Γ£à";
...
function populateTemplateSelect(forceSelectVal) {
    ...
    let html = `
        <optgroup label="Core Letters">
            <option value="offer">≡ƒôä Offer Letter</option>
            <option value="appt">≡ƒôï Appointment Letter</option>
            <option value="confirm">Γ£à Confirmation Letter</option>
            <option value="revised_salary">≡ƒÆ░ Revised Salary Letter</option>
            <option value="incentive">≡ƒÄ» Incentive Circular</option>
        </optgroup>
    `;
    ...
    html += `<option value="create_new" style="font-weight:bold; color:var(--primary);">Γ₧ò Create New Misc Letter...</option>`;
    sel.innerHTML = html;
...
function fillEditorWithRealData(skipConfirm = false) {
    ...
    if (!masterBase || masterBase.trim() === '') {
        return showToast("ΓÜá∩╕Å No master template found. Please create and save a template with {{PLACEHOLDERS}} first.", "warning");
    }
...
async function saveActiveTemplate() {
    ...
    try {
        ...
        await submitProfileUpdate(data);
        showToast("Γ£à Template Saved!", "success");
    } catch (e) {
        showToast("Γ¥î Template Save Failed", "error");
    } finally {
...
async function addDivision(source = 'setup') {
    ...
    try {
        ...
        if (result.success) {
            ...
            showToast(`Γ£à Division "${name}" added`, "success");
            await populateDivisions(true); // Force refresh
        } else {
            showToast("Γ¥î Failed to add division", "error");
        }
    } catch (e) {
        showToast("Γ¥î Communication error", "error");
    } finally {
...
async function addHQ() {
    ...
    try {
        ...
        if (result.success) {
            if (input) input.value = "";
            showToast(`Γ£à HQ "${name}" added`, "success");
            await populateHQs(true);
        }
    } catch (e) { 
        showToast("Γ¥î HQ addition failed", "error"); 
    } finally {
...
async function previewActiveTemplate() {
    ...
    try {
        ...
        if (pdfData && pdfData.doc) {
            ...
            showToast("Γ£à PDF Preview Generated", "success");
            ...
            showToast("Γ¥î Generation failed", "error");
        }
        
    } catch (e) {
        console.error("Live Preview Error:", e);
        showToast("Γ¥î Preview Generation Failed", "error");
    } finally {
...
async function emailLetter(email, type) {
    ...
    try {
        ...
        if ((await res.json()).success) {
            ...
            showToast("Γ¥î Email failed.", "error");
        }
    } catch (e) { showToast("Γ¥î Server error.", "error"); }
...
async function generateLetterPDF(emailOrApp, type, htmlOverride = null) {
    ...
    try {
        ...
        if (!app) {
            showToast("ΓÜá∩╕Å Applicant record not found.", "error");
            return null;
        }
        ...
        if (!template) {
            showToast("ΓÜá∩╕Å Letter template is missing in Setup.", "error");
            return null;
        }
        ...
        showToast("Γ¥î PDF Generation Failed", "error");
        unlockUI();
        return null;
    }
}
...
async function viewDocument(idOrData) {
    ...
    if (!idOrData.startsWith('data:')) {
        try {
            ...
            win.document.body.innerHTML = "Γ¥î Failed to load document data. It may have been cleared or the connection was lost.";
            return;
        }
    }
...
async function downloadAsset(idOrData, name) {
    ...
    if (!idOrData.startsWith('data:')) {
        try {
            ...
            return showToast("Γ¥î Download failed", "error");
        }
    }
...
async function vacuumAssets() {
    ...
    try {
        ...
        const result = await res.json();
        if (result.success) {
            showToast("Γ£¿ Assets vacuumed successfully!", "success");
            await fetchCompanyData();
            renderAssetLists();
        }
    } catch (e) { showToast("Γ¥î Vacuum failed", "error"); }
    finally { unlockUI(); }
}
        ...
        if (alerts && alerts.length > 0) {
            ...
            list.innerHTML = alerts.map(a => `
                <div class="dash-alert ${a.type === 'APPOINTMENT_PENDING' ? 'info' : 'warning'}" style="margin:0; padding:10px 15px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:8px; display:flex; justify-content:space-between; align-items:center;">
                    <span style="font-size:0.9rem; color: #fff;">${a.type === 'APPOINTMENT_PENDING' ? '≡ƒôà' : '≡ƒò╡∩╕Å'} ${a.message}</span>
...
async function publishLetterToHub() {
    ...
    try {
        ...
        const result = await res.json();
        if (result.success) {
            showToast(`Γ£à ${type.toUpperCase()} published successfully!`, "success");
...
async function verifyMaintenanceAuth() {
    ...
    if (!username || !password) {
        return showToast("ΓÜá∩╕Å Admin ID and Password are required", "warning");
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            showToast("Γ¥î Re-authentication failed. Incorrect credentials.", "error");
            document.getElementById('reauthPass').value = '';
        }
    } catch (e) {
        showToast("Γ¥î Server Error during re-auth", "error");
    } finally {
        unlockUI();
    }
}
