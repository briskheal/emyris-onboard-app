...
// 1. Add state hooks for search and division inside SetTargetTab
c = c.replace(
...
// 2. Add handleDownloadYearly and filtered targets logic
const logicInjection = `
  const filteredYearlyTargets = yearlyTargets.filter(t => {
    ...
    const matchesDiv = !yearlyDivision || t.division === yearlyDivision;
...
// 3. Replace the JSX block for Yearly Targets
const oldJsxStart = '<div className="bg-slate-800/80 rounded-2xl border border-slate-700 relative shadow-xl p-8 max-w-5xl mx-auto">';
const oldJsxEnd = '{activeSubTab === \'upload\' && (';

const newJsx = `
<div className="bg-slate-800/80 rounded-2xl border border-slate-700 relative shadow-xl p-8 max-w-[95%] mx-auto w-full">
...
let startIndex = c.indexOf(oldJsxStart);
let endIndex = c.indexOf(oldJsxEnd, startIndex);

c = c.substring(0, startIndex) + newJsx + '\n        ' + c.substring(endIndex);

fs.writeFileSync('xla-frontend/src/pages/ManageUsers.tsx', c);
console.log('Successfully patched ManageUsers.tsx for Yearly Targets features.');
