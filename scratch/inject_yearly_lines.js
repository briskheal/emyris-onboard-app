...
// Now to replace the block.
// I will split the file by lines, find the start and end line indices.
const lines = c.split('\n');
const startLineIdx = lines.findIndex(l => l.includes("{activeSubTab === 'yearly' && ("));
let endLineIdx = -1;
...
console.log("Start line:", startLineIdx, "End line:", endLineIdx);

const newLines = `      {activeSubTab === 'yearly' && (
...
const finalLines = [...lines.slice(0, startLineIdx), ...newLines, ...lines.slice(endLineIdx + 1)];

fs.writeFileSync('xla-frontend/src/pages/ManageUsers.tsx', finalLines.join('\n'));
console.log('Successfully patched ManageUsers.tsx for Yearly Targets features safely.');
