...
async function uploadHQs() {
  const wb = xlsx.readFile('REPORTING MODULE/HQS.xlsx');
  ...
  for (const row of data) {
    try {
      ...
      // API is /api/admin/locations/hqs
      // Needs { state: stateName, hqName }
  ...
  console.log(\Finished processing. Success: \, Failed: \\);
  process.exit(0);
}
