...
import { ArrowLeft, ChevronDown, UserPlus, Plus, Link, Trash2, CheckCircle2, XCircle, Clock, CalendarDays } from 'lucide-react';
...
export default function TourProgram() {
  ...
  const days = [
    { date: 1, day: 'SAT', status: 'Allowed', route: 'Hyderabad Vadodara', badge: 'Out-Station', fullDate: '1 Aug 2026' },
    { date: 2, day: 'SUN', status: 'Not Allowed', route: '', badge: '', fullDate: '2 Aug 2026' },
    { date: 3, day: 'MON', status: 'Allowed', route: 'Vadodara Vadodara City Mkt', badge: 'Out-Station', fullDate: '3 Aug 2026' },
  ];
    ...
    <div className="min-h-screen md:h-dvh bg-slate-900 flex flex-col text-slate-100 font-sans pb-24 md:pb-0 relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col px-5 py-4 md:p-8 overflow-y-auto">
        ...
        <div className="hidden md:flex items-center justify-between mb-8">
          <h2 className="text-xl font-black text-white uppercase tracking-wider">Tour Program</h2>
          
          <div className="relative group">
            <button className="flex items-center gap-2 text-sm font-bold text-sky-400 hover:text-sky-300 transition-colors">
              Quick Links <Link size={16} />
            </button>
            <div className="absolute right-0 mt-2 w-48 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-20">
              <div className="p-2 flex flex-col gap-1">
                <button className="text-left text-sm font-semibold text-slate-300 hover:text-sky-400 hover:bg-slate-700/50 px-3 py-2 rounded-lg transition-colors">New Route</button>
                <button className="text-left text-sm font-semibold text-slate-300 hover:text-sky-400 hover:bg-slate-700/50 px-3 py-2 rounded-lg transition-colors">Route List</button>
                <button className="text-left text-sm font-semibold text-slate-300 hover:text-sky-400 hover:bg-slate-700/50 px-3 py-2 rounded-lg transition-colors">Route Approval</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
          ...
          <button className="md:hidden w-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-2xl p-4 flex items-center justify-center gap-2 active:bg-emerald-500/20 transition-colors">
            <UserPlus size={20} />
            <span className="font-bold text-sm">Add Tour Program for another user</span>
          </button>
        </div>
        ...
        <div className="hidden md:flex flex-col flex-1 bg-slate-800/50 border border-slate-700/50 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="px-6 py-4 bg-slate-800/80 border-b border-slate-700/50 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="bg-sky-500/10 text-sky-400 hover:bg-sky-500/20 border border-sky-500/30 px-3 py-1.5 rounded-lg flex items-center gap-1.5 text-xs font-bold transition-colors">
                <Plus size={14} /> Add
              </button>
            </div>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-900/50 border-b border-slate-700/50">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-widest w-40">Area Type</th>
                  <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-widest">Work Area</th>
                  <th className="p-4 text-xs font-bold text-slate-400 uppercase tracking-widest w-24 text-center">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-700/30">
                {days.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="p-4 text-sm font-bold text-white">{item.route}</td>
                    <td className="p-4 text-center">
                      <button className="text-slate-500 hover:text-rose-400 transition-colors p-2 rounded-lg hover:bg-rose-500/10 inline-flex items-center justify-center">
                        <Trash2 size={18} />
                      </button>
                    </td>
                  </tr>
