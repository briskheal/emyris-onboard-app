...
{!editUser && <div className="mb-10 bg-emerald-900/40 border border-emerald-500/50 p-6 rounded-xl">
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div><label className="text-xs text-slate-400 font-bold mb-1 block">DIVISION</label><select name="division" value={formData.division || ''} onChange={handleChange} className="w-full bg-slate-800 border border-slate-700 rounded-lg p-3 text-sm text-white"><option value="">Select Division</option>{Array.from(new Set(yearlyTargets.map(t => t.division).filter(Boolean))).map(div => <option key={div} value={div}>{div}</option>)}</select></div>
